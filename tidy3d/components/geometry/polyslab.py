...
def leggauss(n):
    ...
    g, w = _leggauss(n)
    return g.astype(config.autograd.gradient_dtype_float, copy=False), w.astype(
        config.autograd.gradient_dtype_float, copy=False
    )
...
class PolySlab(base.Planar):
    ...
    def _edge_events_detection(
        ...
        dist_list = (
            dilation
            * np.linspace(
                0, 1, 1 + _N_SAMPLE_POLYGON_INTERSECT, dtype=config.autograd.gradient_dtype_float
            )[1:]
        )
    ...
    def _compute_derivatives(self, derivative_info: DerivativeInfo) -> AutogradFieldMap:
        ...
        interpolators = derivative_info.interpolators or derivative_info.create_interpolators(
            dtype=config.autograd.gradient_dtype_float
        )
    ...
    def _z_slices(self, sim_min: np.ndarray, sim_max: np.ndarray, is_2d: bool, dx: float):
        ...
        """
        if is_2d:
            zc = np.array([self.center_axis], dtype=config.autograd.gradient_dtype_float)
        ...
        if z1 <= z0:
            return np.array([], dtype=config.autograd.gradient_dtype_float), 0.0, z0, z1

        n_z = max(1, int(np.ceil((z1 - z0) / dx)))
        dz = (z1 - z0) / n_z
        z_centers = np.linspace(
            z0 + dz / 2, z1 - dz / 2, n_z, dtype=config.autograd.gradient_dtype_float
        )
    ...
    def _clip_edge_to_bounds_t(
        ...
        edge_clip_tolerance = config.autograd.edge_clip_tolerance
    ...
    def _adaptive_edge_samples(L: float, dx: float, t_start: float = 0.0, t_end: float = 1.0):
        ...
        sample_fraction = config.autograd.quadrature_sample_fraction
        gauss_quadrature_order = config.autograd.gauss_quadrature_order
        ...
        segs = n_uniform
        edges_t = np.linspace(t_start, t_end, segs + 1, dtype=config.autograd.gradient_dtype_float)
        S, W = [], []
        for i in range(segs):
            ...
            W.append(
                (w_loc * 0.5 * (b - a)).astype(config.autograd.gradient_dtype_float, copy=False)
            )
    ...
    def _collect_sidewall_patches(
        ...
        axis_vec = np.zeros(3, dtype=config.autograd.gradient_dtype_float)
        ...
        if (not is_2d) and len(z_centers) == 0:
            return {
                "centers": np.empty((0, 3), dtype=config.autograd.gradient_dtype_float),
                "normals": np.empty((0, 3), dtype=config.autograd.gradient_dtype_float),
                "perps1": np.empty((0, 3), dtype=config.autograd.gradient_dtype_float),
                "perps2": np.empty((0, 3), dtype=config.autograd.gradient_dtype_float),
                "Ls": np.empty((0,), dtype=config.autograd.gradient_dtype_float),
                "s_vals": np.empty((0,), dtype=config.autograd.gradient_dtype_float),
                "s_weights": np.empty((0,), dtype=config.autograd.gradient_dtype_float),
                "zc_vals": np.empty((0,), dtype=config.autograd.gradient_dtype_float),
                "dz": dz,
        ...
        # pre-allocate arrays
        centers = np.empty((estimated_patches, 3), dtype=config.autograd.gradient_dtype_float)
        normals = np.empty((estimated_patches, 3), dtype=config.autograd.gradient_dtype_float)
        perps1 = np.empty((estimated_patches, 3), dtype=config.autograd.gradient_dtype_float)
        perps2 = np.empty((estimated_patches, 3), dtype=config.autograd.gradient_dtype_float)
        Ls = np.empty((estimated_patches,), dtype=config.autograd.gradient_dtype_float)
        s_vals = np.empty((estimated_patches,), dtype=config.autograd.gradient_dtype_float)
        s_weights = np.empty((estimated_patches,), dtype=config.autograd.gradient_dtype_float)
        zc_vals = np.empty((estimated_patches,), dtype=config.autograd.gradient_dtype_float)
        ...
        sim_min_eff = np.array(sim_min, dtype=config.autograd.gradient_dtype_float)
        sim_max_eff = np.array(sim_max, dtype=config.autograd.gradient_dtype_float)
        for dim in range(3):
            if dim == self.axis:
                continue
            ...
            for zc in z_centers:
                ...
                v0_3d = (
                    self.unpop_axis_vect(
                        np.array([zc], dtype=config.autograd.gradient_dtype_float), v0[None]
                    )[0]
                    + offset3d
                )
                v1_3d = (
                    self.unpop_axis_vect(
                        np.array([zc], dtype=config.autograd.gradient_dtype_float), v1[None]
                    )[0]
                    + offset3d
                )
                ...
                xyz = (
                    self.unpop_axis_vect(
                        np.full(len(s_list), zc, dtype=config.autograd.gradient_dtype_float), pts2d
                    )
                    + offset3d
                )
    ...
    def compute_derivative_slab_bounds_line(
        ...
        coords = np.linspace(
            l_min, l_max, 2 * n_seg + 1, dtype=config.autograd.gradient_dtype_float
        )[1::2]
    ...
    def compute_derivative_slab_bounds_surface(
        ...
        xyz = self.unpop_axis_vect(
            np.full(in_face.sum(), ax_val, dtype=config.autograd.gradient_dtype_float), pts[in_face]
        )
        ...
        normals_xyz = self.unpop_axis_vect(
            ...
            np.zeros((n_patches, 2), dtype=config.autograd.gradient_dtype_float),
        )
        perps1_xyz = self.unpop_axis_vect(
            np.zeros(n_patches, dtype=config.autograd.gradient_dtype_float),
            np.column_stack(
                (
                    np.ones(n_patches, dtype=config.autograd.gradient_dtype_float),
                    np.zeros(n_patches, dtype=config.autograd.gradient_dtype_float),
                )
            ),
        )
    ...
    def _edge_geometry_arrays(self, dtype: np.dtype = config.autograd.gradient_dtype_float):
    ...
    def edge_basis_vectors(
        ...
        edges = edges.astype(config.autograd.gradient_dtype_float, copy=False)

        num_vertices, _ = edges.shape
        zeros = np.zeros(num_vertices, dtype=config.autograd.gradient_dtype_float)
        ones = np.ones(num_vertices, dtype=config.autograd.gradient_dtype_float)
        ...
        axis_norm = np.zeros(3, dtype=config.autograd.gradient_dtype_float)
