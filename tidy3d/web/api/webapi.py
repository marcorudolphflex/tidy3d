...
import tempfile
import time
from typing import Callable, Literal, Optional, Union, TypeAlias
...
def _modesolver_patch(sim: WorkflowType, data: WorkflowDataType) -> None:
    if isinstance(sim, ModeSolver):
        sim._patch_data(data=data)
...
def run(
    ...
    data = load(
        task_id=task_id, path=path, verbose=verbose, progress_callback=progress_callback_download, lazy=lazy
    )
