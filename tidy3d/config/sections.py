...
@register_handler("simulation")
def apply_simulation(config: SimulationConfig) -> None:
    """Apply simulation configuration hooks."""

    if config.use_local_subpixel is not None:
        set_use_local_subpixel(config.use_local_subpixel)


@register_section("autograd")
class AutogradConfig(ConfigSection):
    """Autograd configuration section."""
...
_PREVIOUS_WEB_ENV: dict[str, Optional[str]] = {}
...
class WebConfig(ConfigSection):
    ...
    model_config = ConfigDict(
        json_encoders={ssl.TLSVersion: (lambda value: value.value if value else None)}
    )
